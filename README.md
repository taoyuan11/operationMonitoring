...
游客只能查看公开实例列表中的运行状态和资源指标，不能打开实例详情。CPU/GPU 型号等硬件摘要、
内核、磁盘挂载点、网卡地址和后端观察到的连接 IP 均通过管理员接口展示。设备资料会限制大小并
过滤回环网卡和无效 MAC，不会出现在公开实例列表或指标响应中。升级时应先部署兼容的 backend 和
front-end，再发布 Agent `0.1.21`。
