...
// ExAllocatePool2 is not exported by the Windows 10 1809 kernel. Keep the
// upstream call sites while targeting the oldest supported product release.
#pragma warning(disable : 4996)
#define ExAllocatePool2(poolFlags, byteCount, tag) \
    ExAllocatePoolWithTag(                         \
        (((poolFlags) & POOL_FLAG_PAGED) != 0) ? PagedPool : NonPagedPoolNx, \
        (byteCount),                               \
        (tag))
